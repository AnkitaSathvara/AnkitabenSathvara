...
//   Airports: airportMarkers,
//   Cities: cityMarkers,
//   "Recent earthquakes": earthquakeMarkers,
// };
...
debugger
      if (response) {
        const airports = response.filter((airport) => {
          return airport.name.toLowerCase().includes("airport");
        });

        for (let i = 0; i < Math.min(50, airports.length); i++) {
          L.marker([airports[i].lat, airports[i].lng], {
            icon: airportIcon,
          })

            .bindTooltip(airports[i].name, {
              direction: "top",
              sticky: true,
            })
...
//get citys using open cage and full country name
var cities_list = [];
function getCities() {
  var countryName = $("#countrySelect option:selected").val();
  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      if (result.status.name == "ok" && result.data.current) {
        ...
        $("#todayMaxTemp").html(response.forecast.forecastday[0].day.maxtemp_c);
        $("#todayMinTemp").html(response.forecast.forecastday[0].day.mintemp_c);
        ...
        for (let i = 1; i <= 2; i++) {
          ...
          $(`#day${i}MinTemp`).text(response.forecast.forecastday[i].day.mintemp_c);
          $(`#day${i}MaxTemp`).text(response.forecast.forecastday[i].day.maxtemp_c);
        }

        $("#lastUpdated").text(
          Date.parse(response.current.last_updated).toString(
            "HH:mm, dS MMM"
          )
        );
